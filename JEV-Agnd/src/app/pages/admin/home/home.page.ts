...
export class HomePage implements OnInit {
	...
	indiceDel: any;
	indiceCascata: any;
	apagarService(indice: any, indiceCascata: any) {
		this.indiceDel = indice;
		this.indiceCascata = indiceCascata;

		if (indiceCascata !== '') {
			this.setOpenDeleteCascata(true);
		} else {
	...
	modalOpenDeleteCascata = false;
	setOpenDeleteCascata(isOpen: any) {
		this.modalOpenDeleteCascata = isOpen;
	}
	public alertButtons = [
		{
			text: 'Não',
	...
	public alertInputs = [
		{
			label: 'Apenas este agendamento',
			type: 'radio',
			value: 'OnlyOne',
		},
		{
			label: 'Todos em sequência',
			type: 'radio',
			value: 'AllInRow',
		},
	];
	...
	setResultCascata(ev: any) {
		...
		if (ev.detail.role == 'confirm') {
			console.log(ev.detail.data.values)
			if (ev.detail.data.values === 'OnlyOne') {
				console.log("one")
			...
			if (ev.detail.data.values === 'AllInRow') {
				console.log("all")
